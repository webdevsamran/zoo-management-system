...
if(isset($_POST['submit']))
{
  ...
  $sql="insert into  tblticforeigner (TicketID,NoAdult,NoChildren,AdultUnitprice,ChildUnitprice)values(:ticketid,:adult,:children,:aprice,:cprice)";
  ...
  if ($LastInsertId>0) {
    ...
    echo "<script>window.location.href = 'manage_foreigners_ticket.php'</script>";
  }
  else
  {
...
<form role="form" id=" " method="post" enctype="multipart/form-data">
  <div class="card-body">
    ...
    <?php
    $sql="SELECT * from tbltickettype where TicketType='Foreigner Adult'";
    $query = $dbh -> prepare($sql);
    $query->execute();
    ...
    <?php
    $sql="SELECT * from tbltickettype where TicketType='Foreigner Child'";
    $query = $dbh -> prepare($sql);
    $query->execute();
    $results=$query->fetchAll(PDO::FETCH_OBJ);
    if($query->rowCount() > 0)
